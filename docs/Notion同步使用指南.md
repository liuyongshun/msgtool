...
#### 3.1 公众号（wechat）专用数据库字段

公众号内容会单独写入 `wechat` 数据库，建议字段：

| 字段名 | 类型 | 说明 | 必需 |
|--------|------|------|------|
| Title | Title | 公众号文章标题 | ✅ |
| Summary | Rich text | 正文前 500 字预览 | |
| Source URL | URL | 原文链接（选题来源） | |
| Source Type | Select | 固定为 `wechat` | |
| Article Tag | Select | 固定为 `公众号文章` | |
| Published Date | Date | 生成时间（当天日期） | |

> 说明：完整正文不会放在 Summary 中，而是写在页面正文的 blocks 里（见后文）。
...
```json
{
  "notion_sync": {
    ...
    "databases": {
      ...
      "wechat": {
        "database_id": "你的公众号数据库ID",
        "description": "公众号文章数据库（存储 AI 生成的公众号内容）"
      }
    }
  }
}
...
公众号内容的同步入口在 `http://localhost:5001/wechat`：

1. 浏览器访问 `/wechat`，选择今天的选题并点击「创作内容」
2. 在弹窗中生成公众号正文（Markdown）
3. 预览无误后，点击右下角「同步到 Notion」
4. 成功后，会返回 Notion 页面链接，可直接点击跳转

**正文在 Notion 中的存储方式：**
- 页面的属性区只存 `Title / Summary / Source URL / Source Type / Article Tag / Published Date`
- 真正的全文正文：
  - 写在页面下方的多个 `code` 类型 block 中，语言为 `markdown`
  - 为满足 Notion 限制，每个 block 的单段文本长度 ≤ 2000 字
  - 系统会自动按换行切片，保持 5000 字以内的内容完整同步
